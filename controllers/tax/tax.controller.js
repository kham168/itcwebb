...
const readSingle = async(ID,LocationID, UserLocation, Updateby,Role)=>{
    ...
    try 
    {
        const resultSingle = await dbExecution(`SELECT*FROM public.fn_tax_dql (p_id :=$1,p_locationid :=$2,p_userlocation :=$3,p_updateby :=$4,p_role :=$5,p_action :=$6)`,[ID, LocationID, UserLocation, Updateby,Role,'ONE']);
        
        if(!resultSingle || resultSingle?.rowCount < 1)
        {
            console.log(`resultSingle => ${customDate(new Date())?.formating?.yyyy_dash_MM_dash_dd_space_h24_mm_ss}\tNot fond data`,resultSingle);
            return false;
        }
...
export const createTax = async(req, res) => {
    if(!req?.body?.TaxName || !req?.body?.TaxPercentage || !req?.body?.LocationID)
    {
    ...
    try 
    {
        ...
        const result = await dbExecution(`SELECT public.fn_tax_dml(p_taxname :=$1,p_taxpercentage :=$2, p_note :=$3,p_flag :=$4,p_locationid :=$5,p_userlocation :=$6,p_updateby :=$7, p_role :=$8, p_action := $9)`, inputArr);
...
export const updateTax = async(req, res) => {
    ...
    try 
    {
        ...
        const result = await dbExecution(`SELECT public.fn_tax_dml(p_id :=$1,p_taxname :=$2,p_taxpercentage :=$3, p_note :=$4,p_flag :=$5,p_locationid :=$6,p_userlocation :=$7,p_updateby :=$8, p_role :=$9, p_action := $10)`, inputArr);
...
export const deleteTax = async(req, res) => {
    ...
    try 
    {
        ...
        const result = await dbExecution(`SELECT public.fn_tax_dml (p_id :=$1,p_updateby :=$2,p_locationid :=$3,p_userlocation :=$4,p_role := $5, p_action :=$6)`, inputArr);
...
export const changeTaxStatus = async(req, res) => {
    ...
    try 
    {
        ...
        const result = await dbExecution(`select public.fn_tax_dml (p_id :=$1,p_flag := $2,p_updateby :=$3,p_locationid :=$4,p_userlocation :=$5,p_role := $6, p_action :=$7)`, inputArr);
...
export const readTax = async(req, res) => {
    if(!req?.body?.Action)
    {
    ...
    try 
    {
        ...
        {
            sql = `SELECT*FROM public.fn_tax_dql (p_updateby :=$1,p_role :=$2,p_userlocation :=$3,p_action :=$4)`;
        ...
        {
            sql = `SELECT*FROM public.fn_tax_dql(p_page_number :=$1,p_items_per_page :=$2,p_updateby :=$3,p_role :=$4,p_userlocation :=$5,p_action :=$6)`;
        ...
        else if(req?.body?.Action === "ONE")
        {
            ...
            sql = `SELECT*FROM public.fn_tax_dql(p_id :=$1,p_updateby :=$2,p_role :=$3,p_userlocation :=$4,p_action :=$5)`;
        ...
        else if(req?.body?.Action === "SEARCH")
        {
            ...
            sql = `SELECT*FROM public.fn_tax_dql(p_page_number :=$1,p_items_per_page :=$2, p_keyword :=$3,p_updateby :=$4,p_role :=$5,p_userlocation :=$6,p_action :=$7)`;
